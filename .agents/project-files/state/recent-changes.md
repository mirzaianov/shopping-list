...
- 2026-07-02: Moved the login-owned `SignInView` and CSS module from `src/components` into `src/features/login`, leaving shared button UI in `src/components`. [Reason why added: keeps feature-owned auth views beside their page clients after the sign-up folder split.]
