...
- Two-factor authentication: the feature branch implements optional Better Auth TOTP enrollment, encrypted one-time backup codes, a 10-minute sign-in challenge, optional 30-day trusted devices, and account lockout after 10 failed attempts for 15 minutes. Settings supports setup, backup-code regeneration, and disable flows with password confirmation; `/two-factor` handles authenticator and backup-code challenges. Migration `0005_mature_cannonball.sql` was applied in the user's test environment, and the complete enrollment, challenge, recovery-code, trusted-device, lockout, disable, and session-revocation flow was manually accepted on 2026-07-21. Production migration and deployment remain pending. [Reason why added: distinguishes completed implementation and acceptance from the pending production rollout.]
