...
- UI component system: Base UI is the default headless component layer for new or reworked interactive controls, including shared buttons, menus, dialogs, fields, and icon-control tooltips, while CSS Modules continue to own visual styling. [Reason why added: records the accepted UI-system baseline and current primitive coverage.]
